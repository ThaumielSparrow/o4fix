...
out = pathlib.Path('target/experiments/generalization-v1') / clip
...
source = json.loads((out / 'video-info.json').read_text())
for variant in ['baseline', 'edgeoffset']:
    info = probe(out / f'{variant}-render.mp4')
...
for label, start, duration in windows:
    ...
    subprocess.run([
        ffmpeg, '-v', 'error', '-n', '-ss', str(start), '-i', str(out / 'baseline-render.mp4'),
        '-ss', str(start), '-i', str(out / 'edgeoffset-render.mp4'), '-t', str(duration),
