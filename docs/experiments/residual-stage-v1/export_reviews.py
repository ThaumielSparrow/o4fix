...
windows = {
    ...
    '0073': [('early', 14, 16), ('late', 132, 14), ('control', 55.9, 3), ('extra_rebases', 264, 8)],
}[clip]
out = pathlib.Path('target/experiments/residual-stage-v1') / clip
...
for path in [baseline/'edgeoffset-render.mp4',out/'trustrebase-render.mp4']:
    info=probe(path)
...
for label, start, duration in windows:
    ...
    subprocess.run([
        ...
        '-ss', str(start), '-i', str(out / 'trustrebase-render.mp4'), '-t', str(duration),
