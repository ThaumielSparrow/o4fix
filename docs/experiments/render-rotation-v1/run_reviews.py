...
D=Path('target/experiments/render-rotation-v1')
for name in sys.argv[1:]:
    start={'late':132,'control':55.9,'additional':264}[name]
    full=D/f'compare_{name}_full.mp4'
    if not full.exists():
        subprocess.run(['target/release/examples/render_rotation_review.exe',f'target/experiments/render-acceleration-v2/{name}.mkv',str(D/f'{name}-probe.json'),str(full),f'target/experiments/render-acceleration-v1/compare_{name}.mp4.json'],check=True)
