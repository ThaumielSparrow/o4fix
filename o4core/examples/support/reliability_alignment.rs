//! Frozen research copy of alignment scorer, exposing per-sample diagnostics only.
...
impl CalibrationData {
    pub fn residual_rows(&self, alignment: &Alignment) -> Vec<(usize, f64, [f64; 3], [f64; 3])> {
        ...
        labels
            ...
            .map(|(((run, t), opt), gyro)| {
                let mapped =
                    std::array::from_fn(|c| (0..3).map(|k| opt[k] * alignment.n[k][c]).sum());
                (run, t, mapped, gyro)
            })
            .collect()
    }
