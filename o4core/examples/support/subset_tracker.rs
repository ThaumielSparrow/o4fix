//! Read-only full-fit parity plus disjoint feature-subset stability.
...
fn pair_rotation(
    ...
    let pixels: Vec<_> = good.iter().map(|&i| p0.get(i).unwrap()).collect();
    diagnostic["subset"] = subset::diagnose(
        &pixels,
        &u0,
        &u1,
        prev.cols() as f64,
