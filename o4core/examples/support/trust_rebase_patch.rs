//! Research ablation: additionally rebase explicitly selected zero-trust intervals.
...
#[allow(clippy::too_many_arguments)] // Production signature plus the isolated ablation mask.
pub fn splice_orientation(
    force_rebase: &[(f64, f64)],
    t: &[f64],
    q_raw: &[[f64; 4]],
    omega_patch: &[[f64; 3]],
    intervals: &[(f64, f64)],
    ...
    for &(a, b) in intervals {
        ...
        let rebased = force_rebase.contains(&(a, b))
            || (rebase_above > 0.0 && 1.5 * drift_deg / dur > rebase_above);
        if rebased {
