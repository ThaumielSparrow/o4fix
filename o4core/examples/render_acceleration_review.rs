//! Isolated translation-only review from independently measured triplet acceleration.
use opencv::{
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if a.len() != 4 {
        return Err(
            "usage: render_acceleration_review INPUT_COMPARISON ACC_JSON OUTPUT_MP4".into(),
        );
    }
    ...
    for &correction in &c {
        ...
        let src = Mat::roi(&raw, Rect::new(0, 0, 720, 405))?.try_clone()?;
        let left = warp(&src, [0., 0.])?;
    ...
    std::fs::write(
        format!("{}.json", a[3]),
        serde_json::to_vec(
            &json!({"corrections":c,"max_px":max,"sigma_seconds":0.04,"zoom":1.04,"note":"LEFT common-cropped gap2-edge; RIGHT triplet-acceleration translation correction. No telemetry change. First/last 0.5s not for scoring."}),
        )?,
    )?;
