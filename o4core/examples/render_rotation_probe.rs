...
fn score(points: &[Point2f], acc: &[[f64; 2]]) -> Value {
    ...
    let mut mean_after = 0.;
    let mut roll_after = 0.;
    let mut folds = vec![];
    for c in 0..9 {
        if region[c].len() < 8 {
            continue;
        ...
        let mean: [f64; 2] =
            std::array::from_fn(|k| ids.iter().map(|&i| acc[i][k]).sum::<f64>() / ids.len() as f64);
        let mut fold = [0.; 4];
        for i in 0..points.len() {
            if cell(points[i]) == c {
                ...
                for (k, value) in pred.iter().enumerate() {
                    let rot = if k == 0 {
                        -fit[2] * (points[i].y as f64 - 202.5)
                    } else {
                        fit[2] * (points[i].x as f64 - 360.)
                    };
                    fold[0] += acc[i][k].powi(2);
                    fold[1] += (acc[i][k] - mean[k]).powi(2);
                    fold[2] += (acc[i][k] - value).powi(2);
                    fold[3] += (acc[i][k] - rot).powi(2);
                }
            }
        }
        mean_after += fold[1];
        rigid_after += fold[2];
        roll_after += fold[3];
        folds.push(json!({"region":c,"n":region[c].len(),"alpha":fit[2],"ss":fold}));
    ...
    json!({"n":acc.len(),"shared":center(acc),"split":estimates,"regions":regions,"heldout_before_ss":before,"heldout_after_ss":after,"region_before_ss":region_before,"region_after_ss":region_after,"rigid_region_after_ss":rigid_after,"mean_region_after_ss":mean_after,"roll_region_after_ss":roll_after,"folds":folds,"rigid_fit":rigid_fit(points,acc,&(0..points.len()).collect::<Vec<_>>())})
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() != 5 {
        return Err(
            "usage: render_rotation_probe FULL_VIDEO_OR_COMPARISON START PANEL(0/1) OUTPUT".into(),
        );
    }
