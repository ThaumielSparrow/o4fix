...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if a.len() != 5 {
        return Err(
            "usage: render_rotation_review FULL_VIDEO ROTATION_PROBE OUTPUT_MP4 PRIOR_CORRECTIONS_JSON".into(),
        );
    }
    ...
    std::fs::write(
        format!("{}.json", a[3]),
        serde_json::to_vec(
            &json!({"angles":angle,"max_angle_rad":max,"source_margin":margin,"sigma_seconds":0.04,"band_hz":[4,30],"zoom":1.06,"note":"LEFT prior v1 translation; RIGHT same translation plus rotation-only residual correction. Both from same original render in one warp. No telemetry change. First/last 0.5s not for scoring."}),
        )?,
    )?;
