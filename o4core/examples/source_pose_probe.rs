//! Read-only calibrated source bearing experiment; no repair path.
...
fn interval(
    ...
    for f in 0..count {
        ...
        if let Some(p) = &prev {
            ...
            calib3d::fisheye_undistort_points_def(&to_full(&aa), &mut u0, &k, &d)?;
            calib3d::fisheye_undistort_points_def(&to_full(&bb), &mut u1, &k, &d)?;
            ...
            rows.push(json!({"t":time,"tracks":x.len(),"models":fit.iter().map(|m|m.as_ref().map(|m|json!({"omega":rvec(&m.r).map(|v|v*100.),"support":m.support}))).collect::<Vec<_>>(),"diagnostics":diagnostics,
                "normalized_a":x.iter().map(|p|[p.x,p.y]).collect::<Vec<_>>(),"normalized_b":y.iter().map(|p|[p.x,p.y]).collect::<Vec<_>>(),"ids":labels,"regions":cells}));
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() != 5 {
        return Err("usage: source_pose_probe VIDEO CALIBRATION_JSON BASELINE_OBSERVATIONS_JSON OUTPUT_JSON".into());
    }
    ...
    std::fs::write(
        &args[4],
        serde_json::to_vec(
            &json!({"video":args[1],"metadata":format!("{:?}",tel.meta),"model_order":["rotation_only","essential_small_angle","essential_cheirality","cached_production"],"folds":folds,"rows":rows}),
        )?,
    )?;
    Ok(())
}
