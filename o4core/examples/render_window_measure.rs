...
#[allow(dead_code, clippy::needless_range_loop)] // Existing imported smoother; only pair/gate are used.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if a.len() != 5 {
        return Err("usage: render_window_measure COMPARISON START BURSTS_JSON OUTPUT_JSON".into());
    }
    ...
    let mut prev: Vec<Option<Mat>> = vec![None, None];
    let mut rates = [vec![], vec![]];
    let mut quality = [vec![], vec![]];
    ...
    for k in 0..2 {
        ...
        for (lo, hi) in [(2., 8.), (8., 30.)] {
            ...
            let mut bins = vec![];
            for begin in (0..n).step_by(25) {
                let ids: Vec<_> = (begin..(begin + 25).min(n)).filter(|&i| valid[i]).collect();
                if ids.len() < 10 {
                    continue;
                }
                let xy: f64 = ids
                    .iter()
                    .map(|&i| filtered[i][0].powi(2) + filtered[i][1].powi(2))
                    .sum();
                let ro: f64 = ids.iter().map(|&i| filtered[i][2].powi(2)).sum();
                bins.push(json!({"t":start+begin as f64/fps,"n":ids.len(),"xy":(xy/ids.len() as f64).sqrt(),"roll":(ro/ids.len() as f64).sqrt()}));
            }
            bands.push(json!({"hz":[lo,hi],"bins":bins,"translation_rms_half_px_s":(xy/count as f64).sqrt(),"roll_rms_deg_s":(roll/count as f64).sqrt()}));
        }
    ...
    std::fs::write(
        ...
        serde_json::to_vec_pretty(
            &json!({"window_start":start,"common_active_pairs":count,"panels":panels,"note":"pixel-motion proxy, not ground truth; two panels without additional zoom; confidence neighborhoods and 0.5s edges excluded; no low-frequency pan claim"}),
        )?,
    )?;
    Ok(())
}
