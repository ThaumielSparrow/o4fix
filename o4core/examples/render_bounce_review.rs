//! Bounded 2-8Hz residual translation on top of the visually supported v1 correction.
use opencv::{
    ...
    (0..n)
        .map(|i| {
            ...
            for d in -24i32..=24 {
                ...
                let w = (-0.5 * (d as f64 / 8.).powi(2)).exp();
                total += w;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if a.len() != 5 {
        return Err(
            "usage: render_bounce_review FULL_VIDEO RESIDUAL_PROBE OUTPUT_MP4 PRIOR_CORRECTIONS_JSON".into(),
        );
    }
    ...
    for k in 0..2 {
        let band = o4core::dsp::filtfilt(
            &o4core::dsp::butter_band(2, 2. / 50., 8. / 50.),
            &delta.iter().map(|v| v[k]).collect::<Vec<_>>(),
        );
    ...
    for i in 0..c.len() {
        for (k, (center, last)) in [(720., 1439.), (405., 809.)].iter().enumerate() {
            let shift = 2. * (old[i][k] + c[i][k] / 1.04);
            let lo = center + (0. - center) / 1.06 - shift;
            let hi = center + (last - center) / 1.06 - shift;
            margin = margin.min(lo).min(last - hi);
        }
    }
    if margin < 4. {
    ...
    for (i, &correction) in c.iter().enumerate() {
        ...
        let right = warp(
            &src,
            [
                old[i][0] + correction[0] / 1.04,
                old[i][1] + correction[1] / 1.04,
            ],
        )?;
    ...
    std::fs::write(
        format!("{}.json", a[3]),
        serde_json::to_vec(
            &json!({"residual_corrections":c,"max_px":max,"source_margin":margin,"sigma_seconds":0.08,"band_hz":[2,8],"zoom":1.06,"note":"LEFT prior v1 translation; RIGHT same translation plus 2-8Hz residual correction. Both from same original render in one warp. No telemetry change. First/last 0.5s not for scoring."}),
        )?,
    )?;
