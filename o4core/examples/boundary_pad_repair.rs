...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if a.len() != 6 {
        return Err(
            "usage: boundary_pad_repair SOURCE RAMP019_REFERENCE TRACE DESTINATION METRICS".into(),
        );
    }
    ...
    let expanded = detect::find_intervals(
        &diag
            .noise
            .iter()
            .map(|&v| v > cfg.severe)
    ...
    baseline.require_coverage(&tel.t, &expanded)?;
    ...
    std::fs::write(
        ...
        serde_json::to_vec_pretty(
            &json!({"parity_max_component":parity,"baseline_padding":0.2,"candidate_padding":0.4,"bursts":rows,"windows":windows,"note":"Same cached production rates; padding 0.2 to 0.4; ramp remains 0.19. Intervals and endpoint drift/rebase decisions can change. Research only."}),
        )?,
    )?;
