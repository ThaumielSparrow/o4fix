...
pub fn video_info(video: &Path) -> Result<VideoInfo, O4Error> {
    let cap =
        videoio::VideoCapture::from_file(video.to_str().unwrap_or_default(), videoio::CAP_ANY)?;
    let info = VideoInfo {
...
pub fn window_frames(a: f64, b: f64, info: &VideoInfo) -> (i64, i64) {
    ...
    (
        ((a * info.fps).round() as i64).clamp(0, last),
        ((b * info.fps).round() as i64).clamp(0, last),
    )
}
    ...
    for fi in f0..=f1 {
        ...
        if let Some(p) = &prev {
            ...
            if tr.len() >= 40 {
                ...
                let inside = |v: &V3| {
                    v[2] > 0.0
                        && (v[0] / v[2]).abs() < cfg.crop.0
                        && (v[1] / v[2]).abs() < cfg.crop.1
                };
                ...
                for i in 0..tr.len() {
                    ...
                    wa.push(rotate(
                        orient.at(row_t(fa, tr[i].0.y)),
                        mat_vec(&O4P_MOUNT, ba[i]),
                    ));
                    wb.push(rotate(
                        orient.at(row_t(fi, tr[i].1.y)),
                        mat_vec(&O4P_MOUNT, bb[i]),
                    ));
                }
